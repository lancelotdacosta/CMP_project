...
Mapperplot <- function(Mapper_output){
  ...
  data_description <- Mapper_output[[3]]   
  #filter_values <- Mapper_output[[4]]   
  lens_description <-   Mapper_output[[5]]   
  ...
  #plot title and text
  title(paste("Data: ", data_description)) #Data description
  mtext(paste("Lens: ", lens_description)) #Lens description
}



Mappercolor <- function(Mapper_output, color_lens = NULL, color_lens_description = NULL, method = "mean"){
  library(igraph)
  ...
  if(is.null(color_lens) | is.null(color_lens_description)){
    ...
    color_lens_description <- lens_description
  }
  ...
  for (i in 1:mapper_object$num_vertices){
    points.in.vertex <- mapper_object$points_in_vertex[[i]]
    vertex.size[i] <- length(points.in.vertex)              #1.5 for 100 rows good:change this
  }
  ...
  #Assign a color to each method vertex
  
  vector_of_colors <- rainbow(length(method.vertex))
  
  V(Mapper.graph)$color <- vector_of_colors[order(method.vertex)]
  V(Mapper.graph)$size <- vertex.size
  
  plot(Mapper.graph, main ="Mapper Graph")
  legend(x=-2, y=-1,
         c(paste("low", color_lens_description), paste("medium", color_lens_description), paste("high", color_lens_description)),
