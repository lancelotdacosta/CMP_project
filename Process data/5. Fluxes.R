...
#Function
fluxes.cc.data <- function(cc.data){
#list to store data
list.of.dataframes <- vector("list", length(unique(cc.data$Tree))*length(unique(cc.data$RF))) #we again assume that all trees are always sampled with the same amount of radial files
#a counter
i <- 1
