...
#Add samples with NAs for missing samples
#Replace NAs by coherent values when there not all measurements in a radial file are NAs
#When there are only NAs, replace by linear interpolation
#-> the cell count data is now clean and complete
#incorporate meteorological data and cell count data in the same data-frame df
#linear interpolate elements of this dataframe df

#Note: we did not exploit the precision column in the cell count data as we do not know how to interpolate it
...
plotree <- function(n)
{
  d <- filter(df, Tree ==n)
  plot(d$jour, d$MZ)
  lines(d$jour,d$CZ,type = "p", col= "green")
  lines(d$jour,d$EZ,type = "p", col= "orange")
  lines(d$jour,d$WZ,type = "p", col= "red")
}
...
#STEP2###############Replace missing values in cell count data
...
#######Test results
for(i in 41:55){plotree(i)}
# Trees 55,54,53,52,51,45 have mature cells going down at the end.

#replaces integers by numeric
df$jour <- as.numeric(df$jour)

#saves the document
setwd("/Users/lancelotdacosta/CMP project/ABR07")
save(df, "df", file = "cell_count_ABR07.RData")

#our big data frame ABR07 is saved