#3. Linear interpolate cell count data to get daily cell count data throughtout the year
...
#Function
linear.interpolate.cc.data <- function(cc.data){
...
#we find the number of days in the given year and store it in days_in_year
year <- cc.data$Year[1]
next_year <- paste(c(as.character(year+1),"01","01"), collapse = "-")
this_year <- paste(c(as.character(year),"01","01"), collapse = "-")
days_in_year <- as.numeric(as.Date(next_year) - as.Date(this_year))
